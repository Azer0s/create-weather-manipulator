...
- **Names and numbers:** right-click a placed link with a named name tag to name it; unnamed links are automatically handed "Charger 1", "Charger 2" and so on within their network. Goggles on any linked charger show the whole network's fill bar and numbers, plus which named charger currently holds the discharge lead.
- **Heartbeat:** every link flashes its bulb every two seconds on the display link's own pulse machinery. Links of one network blink in unison; separate networks beat to their own rhythm.
