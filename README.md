...
- **Several chargers:** unlinked chargers do not coordinate. With matching charge speeds they simply push together; with mismatched speeds they fight like any two conflicting sources in Create. To make batteries take turns, wire them into a Charger Link network.
...
- **Coordinated discharging, nothing else:** chargers wearing links of one network take turns as the battery: exactly one (the lowest position with energy) generates at a time, the rest stand by, and when the lead runs dry the next takes over. No energy ever moves between chargers; every buffer stays where it was charged. A bound link item shimmers like an enchanted book so fresh and bound stacks are told apart at a glance.
- **Names and numbers:** right-click a placed link (with an empty hand or anything that is not a link item) to open its naming screen; clearing the name hands the link back its automatic number. Unnamed links are handed "Charger 1", "Charger 2" and so on within their network. Goggles on any linked charger show the whole network's fill bar, its pooled numbers, how long the network lasts at the current draw (while discharging), and which named charger holds the lead.
- **Two lights:** the big bulb is the status lamp, glowing steadily in the host charger's state color: blue while charging, red while discharging, green when full, yellow when idle or standing by. The small LED on the housing is the sync indicator: it flashes red every two seconds on the display link's own pulse machinery, links of one network in unison, separate networks to their own rhythm.
- **Redstone output:** the block emits a signal of 0 to 15 proportional to the buffer fill; a comparator reads the same value, and goggles show the exact numbers, the wheel count, the current mode and how long the charge lasts at the current draw.
- **Display link support:** Create's display link can be placed on a charger and read either the charge level (stored, capacity and percent) or the status (mode and wheel count) onto any display target.
